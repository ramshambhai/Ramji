...
@ServerEndpoint("/configopr")
public class ConfigOprServer
{
	...
	{	
		...
		public  void sendText(HashMap<String,String> packet)
	{	
			fileLogger.info("Inside Function : sendText");			
		String pack = convertMapDataToJson(packet);
		for(Session ses:sessionObjs)
		{
			try
			{
	...
	public String convertMapDataToJson(HashMap<String,String> packet)
	{
		fileLogger.info("Inside Function : convertMapDataToJson");		
		JSONObject jo = new JSONObject();
		try
		{
			for(Map.Entry<String,String> m:packet.entrySet()){  
				jo.put(m.getKey(),m.getValue());
			}  		
		}
		catch(Exception E)
		{
		...
		fileLogger.info("Exit Function : convertMapDataToJson");	
		return jo.toString();
	}
}
